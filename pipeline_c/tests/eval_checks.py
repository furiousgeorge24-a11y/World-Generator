...
def check_schemas_and_prompts() -> tuple[dict[int, dict], dict[int, dict], dict[int, dict]]:
    ...
    check("all scaffold schemas are present", set(schemas) == {
        ...
        "layer_audit_v1.schema.json",
        "layer_panel_key_v1.schema.json",
...
def check_engine_independence() -> None:
    ...
    check("calibration controls are generated, never shipped",
          not list((ROOT / "eval").rglob("*.npy")))
