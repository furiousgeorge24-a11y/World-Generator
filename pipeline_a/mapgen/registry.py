...
REGISTRY: list[Control] = [
    ...
    Control("plateau_tendency", "float", 0.0, 0.0, 1.0, "boundaries", FULL,
            "primary",
            "How readily strong convergence matures into an orogenic "
            "plateau (tier-2 re-grounding: no prescribed fill rows — past "
            "the isostatic ceiling excess shortening widens the thickened "
            "zone, whose span breathes with convergence along strike; "
            "basin fill flattens the interior; the retro rim is the "
            "shortening front at the zone's edge). 0 = peaks only "
            "(ablation); 1 = Tibets form at moderate convergence."),
    ...
    Control("crest_sharpness", "float", 0.6, 0.0, 1.0, "boundaries", FULL,
            ...
            "soft-topped."),
    Control("belt_raggedness", "float", 0.5, 0.0, 1.0, "boundaries", FULL,
            "primary",
            "Along-strike variation of convergent belts: shortening rate "
            "varies along the boundary, so crest height pulses into "
            "saddles and passes, belt width breathes, the axis meanders, "
            "and the foreland trough breaks into basin chains instead of "
            "a straight strip. 0 = uniform ruler-straight belts "
            "(ablation)."),
    Control("crest_zone", "float", 0.6, 0.0, 1.0, "boundaries", FULL,
            "advanced",
            "Summit regions where convergence is fastest: broad clusters "
            "of near-ceiling mass (a Karakoram, not a lone ridgeline) "
            "rather than uniform crest height along the whole belt. "
            "0 = ridgelines only."),
    Control("rift_segmentation", "float", 0.65, 0.0, 1.0, "boundaries",
            FULL, "primary",
            "Continental rifts break into en-echelon half-graben linked "
            "by transfer-zone sills: deep mid-segment basins, shallow "
            "accommodation zones between — lake chains on land, "
            "irregular flooded arms at the coast. 0 = one straight "
            "uniform trough (ablation)."),
    Control("outer_rise", "float", 0.6, 0.0, 1.0, "boundaries", FULL,
            "advanced",
            "Plate flexure under mountain loads: the foreland basin "
            "depressed behind convergent belts (the seaward outer-rise "
            "bulge was trimmed 2026-08-29 as visually inert — author "
            "ruling). 0 disables the flexural basin."),
    ...
    Control("era_count", "int", 1, 1, 3, "eras", FULL, "primary",
            "Tectonic eras stacked into the terrain. 1 = the current era "
            "only — ancient belts OFF, the author-default off position. "
            "2-3 imprint worn, blunted orogens inherited from older plate "
            "configurations inside today's continents."),
    ...
    Control("plains_grain", "float", 0.5, 0.0, 1.0, "relief", FULL,
            "primary",
    ...
    Control("basin_fill", "float", 0.7, 0.0, 1.0, "erosion", FULL,
            "advanced",
            "Steady-state fill of interior depressions: the carved load "
            "each basin's catchment routes into it (metered per basin) "
            "accumulates until floors flatten toward the spill — "
            "sediment plains with residual lakes only in the deepest "
            "hollows, not brimful mega-lakes. Higher values equilibrate "
            "closer to the spill; starved basins always keep their "
            "lakes; never raises ground above the spill. 0 = transient "
            "fill-to-spill lakes (ablation)."),
