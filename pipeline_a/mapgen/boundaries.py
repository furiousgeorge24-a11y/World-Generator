...
def stage_boundaries(world: World) -> None:
    ...
    rag_ctl = float(c["belt_raggedness"])
    zone_ctl = float(c["crest_zone"])
    rseg_ctl = float(c["rift_segmentation"])
    nsalts = salts_for(world.seed, "boundaries:strike", 4)
    msalts = salts_for(world.seed, "boundaries:massif", 6)
    rsalts = salts_for(world.seed, "boundaries:rag", 8)
    ...
    for a, b, pts, nx, ny, vn in all_rows:
        ...
        # belt raggedness (tier 2): shortening rate varies along strike —
        # obliquity, inherited crust — so thickening intensity, belt width,
        # axis position and the foreland trough all pulse along the belt.
        # All four collapse to identity at the knob's zero (ablation).
        tvar = np.clip(1.0 + 1.0 * rag_ctl
                       * fbm(px, py, 520.0, rsalts[0:2]), 0.05, 2.2)
        meand = rag_ctl * (60.0 / cell) * fbm(px, py, 340.0, rsalts[2:4])
        wvar = np.clip(1.0 + 0.55 * rag_ctl
                       * fbm(px, py, 230.0, rsalts[4:6]), 0.35, 1.8)
        fvar = np.clip(1.0 + 1.25 * rag_ctl
                       * fbm(px, py, 330.0, rsalts[6:8]), 0.05, 2.2)

        tx, ty = -ny, nx                          # boundary tangent

        # --- convergent (K2: profile model; tier-2 re-grounding) --------
        # Thickening intensity T (from closing rate, pulsing along strike)
        # sets a *saturating* crest height — the isostatic ceiling — and
        # past a threshold the orogen spreads instead of rising: excess
        # shortening widens the thickened zone (a continuous band whose
        # span breathes with T along strike — no prescribed fill rows),
        # deformation concentrates at the zone's retro edge (the rim
        # front), and the load flexes the retro plate into a foreland
        # basin. Apron rows carry the wide retro flank of an unsaturated
        # belt. Interior flatness is the basin-fill stage's job.
        for m, ovr in ((conv & ca & ~cb, -1.0), (conv & cb & ~ca, +1.0)):
            if m.any():
                ...
                ar = rr[m] + dr + ovr * ny[m] * g + ny[m] * (jit[m] + meand[m])
                ac = cc[m] + dc + ovr * nx[m] * g + nx[m] * (jit[m] + meand[m])
                T = amp[m] * orog * cmod[m] * tvar[m]
                Hs = 6000.0 * T / (T + 0.75)          # saturating height
                pw = np.clip((T - (1.55 - 0.95 * pt_ctl)) / 0.55, 0.0, 1.0)
                Wp = np.minimum((90.0 + 210.0 * pw
                                 + 130.0 * np.clip(T - 1.4, 0.0, 1.2))
                                * wvar[m], 420.0) / cell   # zone span,
                                                  # Altiplano-scale cap
                ...
                if zone_ctl > 0.0:                    # summit regions where
                    zk = np.clip((T - 1.05) / 0.75, 0.0, 1.0) ** 1.5
                    mz = zk > 0.02                    # T nears the ceiling
                    if mz.any():
                        splat(grids["crest_zone"], ar[mz], ac[mz],
                              (0.30 * Hs * zk * kmod[m])[mz] * zone_ctl,
                              _SIGMA["crest_zone"][0])
                splat(grids["volcanic"], ar, ac, amp[m], _SIGMA["volcanic"][0])
                oy = ovr * ny[m]
                ox = ovr * nx[m]
                m2 = pw > 0.05
                stp = 45.0 / cell                     # band sampling only —
                if m2.any():                          # span varies with T
                    kmax = int(np.ceil(float(Wp[m2].max()) / stp))
                    for k in range(1, min(kmax, 12) + 1):
                        ...
                        if mk.any():
                            splat(grids["plateau"], (ar + oy * dk)[mk],
                                  (ac + ox * dk)[mk],
                                  (0.216 * Hs * pw)[mk], _SIGMA["plateau"][0])
                rim2 = Wp + 40.0 / cell               # retro shortening front
                if m2.any():
                ...
                splat(grids["foreland"], ar + oy * d_fb, ac + ox * d_fb,
                      -0.05 * Hs * rise_ctl * fvar[m], _SIGMA["foreland"][0])
        ...
        if m.any():
            ...
            jr = ny[m] * (jit[m] + meand[m])
            jc = nx[m] * (jit[m] + meand[m])
            T = amp[m] * orog * cmod[m] * tvar[m]
            Hs = 6300.0 * T / (T + 0.70)            # collisions run higher
            pw = np.clip((T - (1.15 - 0.85 * pt_ctl)) / 0.5, 0.0, 1.0)
            Wp = np.minimum((75.0 + 175.0 * pw      # spreads on both sides
                             + 110.0 * np.clip(T - 1.2, 0.0, 1.2))
                            * wvar[m], 450.0) / cell   # Tibet-scale/side
            ...
            splat(grids["crest"], rr[m] + jr, cc[m] + jc,
                  0.22 * Hs * kmod[m] * crest_ctl, _SIGMA["crest"][0])
            if zone_ctl > 0.0:
                zk = np.clip((T - 0.95) / 0.75, 0.0, 1.0) ** 1.5
                mz = zk > 0.02
                if mz.any():
                    splat(grids["crest_zone"], (rr[m] + jr)[mz],
                          (cc[m] + jc)[mz],
                          (0.30 * Hs * zk * kmod[m])[mz] * zone_ctl,
                          _SIGMA["crest_zone"][0])
            m2 = pw > 0.05
            ...
            for s in (-1.0, 1.0):
                sy = s * ny[m]
                sx = s * nx[m]
                for k in range(1, min(kmax, 12) + 1):   # thickened zone
                    dk = k * stp                        # (span breathes)
                    mk = m2 & (dk <= Wp)
                    if mk.any():
                        splat(grids["plateau"], (rr[m] + jr + sy * dk)[mk],
                              (cc[m] + jc + sx * dk)[mk],
                              (0.219 * Hs * pw)[mk], _SIGMA["plateau"][0])
                if m2.any():                        # rim shortening fronts
                ...
                splat(grids["foreland"], rr[m] + sy * d_fb,
                      cc[m] + sx * d_fb, -0.045 * Hs * rise_ctl * fvar[m],
                      _SIGMA["foreland"][0])
            splat(grids["orient_c"], rr[m], cc[m],
                  (tx[m] ** 2 - ty[m] ** 2) * amp[m], _SIGMA["orient_c"][0])
            splat(grids["orient_s"], rr[m], cc[m],
        ...
        if m.any():
            ...
            # tier 2: en-echelon segmentation — extension localizes on
            # half-graben segments linked by transfer-zone sills (East
            # African grammar): segments step sideways, depth pulses to
            # shallow sills between them. env/offsets are identity at 0.
            nm2 = int(m.sum())
            drr = np.zeros(nm2)
            dcc = np.zeros(nm2)
            env = np.ones(nm2)
            if rseg_ctl > 0.0 and length_km > 240.0:
                seg_len = 200.0 + 140.0 * prng.random()
                n_seg = max(2, int(round(length_km / seg_len)))
                offs = ((16.0 + 26.0 * prng.random(n_seg))
                        * np.where(prng.random(n_seg) < 0.5, 1.0, -1.0)
                        * rseg_ctl / cell)
                u = np.clip((t[m] + big_t) / (2 * big_t) * n_seg,
                            0.0, n_seg - 1e-6)
                kseg = u.astype(int)
                frac = u - kseg
                ...
                drr = nmy * offs[kseg]
                dcc = nmx * offs[kseg]
                pulse = np.sin(np.pi * frac) ** 0.7
                env = (1.0 - rseg_ctl) + rseg_ctl * (0.22 + 0.78 * pulse)
            gr = rr[m] + drr
            gc = cc[m] + dcc
            if stagev < 0.45:                       # young valley
                splat(grids["graben"], gr, gc, -680.0 * amp[m] * env,
                      _SIGMA["graben"][0])
                sh_amp = 430.0
            elif stagev < 0.75:                     # lake chain
                splat(grids["graben"], gr, gc, -1050.0 * amp[m] * env,
                      _SIGMA["graben"][0])
                sh_amp = 300.0
            else:                                   # narrow sea: a shallow
                splat(grids["graben"], gr, gc,     # syn-rift-filled floor
                      -700.0 * amp[m] * env,       # with a deeper axial
                      _SIGMA["graben"][0])         # trough mid-segment
                splat(grids["graben_w"], gr, gc,
                      -950.0 * amp[m] * (0.7 + 0.5 * cmod[m])
                      * (0.55 + 0.45 * env),
                      _SIGMA["graben_w"][0])
                sh_amp = 0.0
            if sh_amp > 0.0:
                g = 42.0 / cell
                for s in (-1.0, 1.0):
                    splat(grids["shoulder"], gr + s * ny[m] * g,
                          gc + s * nx[m] * g,
                          sh_amp * amp[m] * orog * (0.6 + 0.4 * env),
                          _SIGMA["shoulder"][0])
    ...
    pos = (fields["cordillera"] + fields["cordillera_n"] + fields["crest"]
           + fields["crest_zone"] + fields["plateau"] + fields["apron"]
           + fields["arc_oo"] + fields["arc_comp"] + fields["arc_ped"]
           + fields["shoulder"] + fields["hotspot"] + fields["hotspot_comp"]
           + fields["hotspot_ped"])
