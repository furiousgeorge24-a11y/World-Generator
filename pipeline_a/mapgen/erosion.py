...
from .hydrology import (fill_depressions, flow_accumulation,
                        flow_directions, label_lakes)
...
def stage_erosion(world: World) -> None:
    ...
    if strength > 0.0 and steps > 0:
        ...
        e0 = e.copy()
        dep_t = basin_t = exp_t = fill_t = 0.0
        ...
        # --- tier 2: steady-state intermontane basin fill ---------------
        # A depression is a sediment trap: over the ~My this snapshot
        # stands for, the load its catchment delivers accumulates until
        # the floor flattens toward the spill — a sediment plain with
        # residual lakes only in the deepest hollows. Supply is each
        # basin's real influx: the carved mass routed into it on the
        # filled surface (routing crosses lakes toward the spill, so the
        # peak through-flux at the basin IS its catchment load).
        # _FILL_AMP stands in for the accumulation time the way B1's
        # _RISE_AMP does for the rise apron; starved basins keep their
        # lakes. Fill is proportional to local depth (lacustrine fill is
        # thickest over the deepest floor) and capped below the spill —
        # never new ridges, never land the basin didn't already enclose.
        bas_ctl = float(c["basin_fill"])
        if bas_ctl > 0.0:
            Fb, _ = fill_depressions(e)
            depthb = np.maximum(Fb - e, 0.0) * (e >= 0.0)
            if float(depthb.max()) > 0.5:
                _FILL_AMP = 60.0
                d8b, rvb = flow_directions(Fb, cell)
                eroded_m = np.maximum(e0 - e, 0.0)
                acc_e = eroded_m.ravel().tolist()     # routed carved mass
                rl = rvb.ravel().tolist()
                for i in np.argsort(Fb, axis=None,
                                    kind="stable")[::-1].tolist():
                    j = rl[i]
                    if j >= 0:
                        acc_e[j] += acc_e[i]
                acc_e = np.asarray(acc_e).reshape(world.shape)
                ids, nb = label_lakes(depthb > 0.01)
                v_dep = np.bincount(ids.ravel(), weights=depthb.ravel(),
                                    minlength=nb + 1)
                influx = np.zeros(nb + 1)
                np.maximum.at(influx, ids.ravel(), acc_e.ravel())
                # supply gates starved basins; for the (common) well-fed
                # rest the knob sets how far short of the spill the fill
                # equilibrates — the abstracted subsidence/water balance
                f_b = np.minimum(_FILL_AMP * bas_ctl * influx
                                 / np.maximum(v_dep, 1e-9),
                                 0.95 * bas_ctl ** 0.7)
                f_b[0] = 0.0
                fill = f_b[ids] * depthb
                e += fill
                fill_t = float(fill.sum())
        ...
        world.findings.append(
            {"check": "erosion", "level": "info", "steps": steps,
             ...
             "basin_fill_km3": round(fill_t * vol, 1),
