...
- **Tier 2 — belt-and-basin anatomy: authorized and implemented at
  0.10.0** (2026-08-29; scope expanded 2026-08-28 after the author
  flagged inland-water anatomy: belt mega-lakes read wrong — many,
  massive, smooth — vs ref7's small irregular chains; the rift graben
  reads as a straight abyssal "laser". Diagnosis: K2 troughs were
  unsegmented along-strike, fill-to-spill lakes are maximal by
  construction, and the deposit pass's pit-trap branch never fires on
  the filled routing surface — we rendered the transient state).
  Scope sitting picks: lake palette quieting **held** until this
  review shows whether the causal fixes suffice; outer-rise seaward
  bulge **trimmed** (knob now foreland-flexure-only); single end
  review. Six mechanisms, four new controls, all ablation = knob at
  zero:
  **belt raggedness** (`belt_raggedness`): T, belt width, axis
  position (±60 km meander) and the foreland trough pulse along
  strike; era belts inherit it. **Plateau re-grounding** (rides
  `plateau_tendency`, default still 0 pending the author's ladder
  ruling): fill rows deleted — the thickened zone is a continuous
  band sampled at 45 km (invisible under the σ60 kernel) whose span
  grows with excess T past saturation (90–420 km oc-ct, ≤450/side
  collision, Altiplano/Tibet caps) and breathes along strike; the
  retro rim is the shortening front at the zone's true edge; interior
  flatness is basin fill's job. **Intermontane basin fill**
  (`basin_fill`): a fresh depression fill after the carve; each
  depression fills toward its spill by the carved load its own
  catchment routes into it (per-basin metering via routed-mass
  accumulation on the filled surface — the honest replacement for the
  never-firing sink-trap signal), proportional to local depth
  (lacustrine fill is thickest over the deepest floor), capped below
  the spill; the knob sets equilibration headroom (abstracted
  subsidence/water balance), starved basins keep their lakes.
  Guards: never moves a coastline, raise-only, never above spill.
  **Foreland along-strike modulation**: trough amplitude ±125%
  (fvar) — the straight foreland lake strips become basin chains.
  **Rift segmentation** (`rift_segmentation`): en-echelon half-graben
  (200–340 km segments, ±16–42 km lateral steps, random polarity)
  linked by transfer-zone sills (depth envelope 0.22–1.0); the
  narrow-sea wide floor shallows (−1500 → −950, sill-damped) so
  flooded arms read shallow-with-axial-deeps, not abyssal — EAR
  grammar on land, Red Sea at coasts. **Crest-zone mass**
  (`crest_zone`): broad σ46 km summit regions where T nears the
  ceiling (superlinear gate), bounded by the isostatic knee.
  Untouched by all of this: the drowned-shallows/fragmented-coast
  look the author blessed. M3's evaporative lake levels remain the
  later, causal authority on lake *levels* (deferred by design).
  Evidence: out/tier2_anatomy/ (audit pairs, four ladders, rift/basin
  fixed-spot zooms, knock-outs, seeds, 1024² money shots). Smoke
  41/41 (8 new guards).
...
| # | Question | Status |
|---|---|---|
| 1 | Eras (history option C) authorized as extension? | closed 2026-08-29 — retained, ruled process-clean (real ancient kinematics), but **default off** (era_count 1, primary slider); author tinkers later. Banks ~2 s at 1024² |
